# Smart-Mixin
## An open source Clash configuration preprocessor.
## Tips
### Filter Nodes
```Python
select_all(CONF.Proxies, False, re_name = "Official Website").delete()
```
Remove nodes with regular matches to names from the proxy list and all proxy groups, equivalent to filtering nodes.
## Change Log
### 2.0.5
The update includes code logic and performance optimizations.
### 2.0.4
The way `Config` is instantiated has been changed: the original `url` parameter name has been changed to `Url`; `path` is replaced with `File` argument (file object), solving the problem of encoding causing reading errors.  
We have introduced templates! This is a quick way to create the configuration files you want, and the templates will be expanded in the future.  
Added usage tips to `Readme`.
### 2.0.3
Fixed an issue where the proxy in `DICT` could not be a proxy type when `ProxyGroup` was initialized; this fix has the potential to introduce other potential issues.
### 2.0.2
Fixed the infinite loop when renaming `Proxy` with the same original name and new name.  
Fixed the problem that the renaming of `ProxyGroup` causes prasing exceptions.
### 2.0.1
Added `Mixin` function.  
`ReadMe` minor change.
### 2.0.0
This is the first realease of 2.x.x version.
